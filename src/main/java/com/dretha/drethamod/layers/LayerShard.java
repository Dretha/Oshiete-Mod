...
public class LayerShard implements LayerRenderer<EntityLivingBase>
{

    private RenderLivingBase<?> renderer = null;

    public LayerShard(RenderLivingBase<?> rendererIn)
    {
    ...
    public void doRenderLayer(EntityLivingBase base, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
	    ...
	    int shards = stats.getShardCountInEntity();

        if (shards > 0)
        {
            Entity entity = new EntityRCShard(base.world, base.posX, base.posY, base.posZ);
            Random random = new Random(base.getEntityId()+1);
            ...
            for (int j = 0; j < shards; ++j)
            {
                ...
                if (base instanceof EntityPlayer) {
                    renderer = Minecraft.getMinecraft().getRenderManager().getSkinMap().get(((AbstractClientPlayer) base).getSkinType());
                }
        ...
        if (base.ticksExisted%2000==0) stats.removeShardCountInEntity();
    }
